...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE INDEX \`users_rels_websites_id_idx\` ON \`users_rels\` (\`websites_id\`);`)
  await db.run(sql`CREATE TABLE \`websites\` (
  	...
  	\`settings_secondary_color\` text,
  	\`settings_analytics_id\` text,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	FOREIGN KEY (\`settings_logo_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`websites_domain_idx\` ON \`websites\` (\`domain\`);`)
  await db.run(sql`CREATE UNIQUE INDEX \`websites_slug_idx\` ON \`websites\` (\`slug\`);`)
  await db.run(sql`CREATE INDEX \`websites_settings_settings_logo_idx\` ON \`websites\` (\`settings_logo_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`pages_website_idx\` ON \`pages\` (\`website_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`_pages_v\` (
  	...
  	FOREIGN KEY (\`version_website_id\`) REFERENCES \`websites\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`_pages_v_parent_idx\` ON \`_pages_v\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_pages_v_version_version_slug_idx\` ON \`_pages_v\` (\`version_slug\`);`)
  await db.run(sql`CREATE INDEX \`_pages_v_version_version_website_idx\` ON \`_pages_v\` (\`version_website_id\`);`)
  ...
  await db.run(sql`ALTER TABLE \`users\` ADD \`role\` text DEFAULT 'viewer';`)
  await db.run(sql`ALTER TABLE \`users\` ADD \`default_website_id\` integer REFERENCES websites(id);`)
  await db.run(sql`CREATE INDEX \`users_default_website_idx\` ON \`users\` (\`default_website_id\`);`)
  await db.run(sql`ALTER TABLE \`media\` ADD \`website_id\` integer REFERENCES websites(id);`)
  await db.run(sql`CREATE INDEX \`media_website_idx\` ON \`media\` (\`website_id\`);`)
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`websites_id\` integer REFERENCES websites(id);`)
